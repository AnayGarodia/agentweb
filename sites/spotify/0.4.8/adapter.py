...
class Adapter(SiteAdapter):
    ...
    def playlist(
        ...
        if not self._has_oauth_tokens() and self._has_web_session():
            ...
            # Expose the true privacy state so `public` can be verified from a
            # read (the QA gap: create/update reported public but reads had no
            # field to confirm it). Prefer the permission carried in the fetch
            # payload; fall back to the permission service only when absent.
            base_permission = str(payload.get("basePermission") or "")
            permission_elapsed = 0.0
            if not base_permission:
                try:
                    perm, permission_elapsed = self._web_playlist_permission(
                        normalized_id, None
                    )
                    base_permission = str(perm.get("permission_level") or "")
                except Exception:
                    base_permission = ""
            if base_permission and base_permission != "UNKNOWN":
            ...
            return {
                ...
                "control_path": "retained_web_session_replay",
                "elapsed_ms": round(elapsed + permission_elapsed, 1),
            }
