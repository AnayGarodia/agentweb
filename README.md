...
| BSE | `bseindia.com` | 0.3.0 | 4 | all verified | none |
| CoinGecko | `coingecko.com` | 0.2.0 | 3 | all verified | none |
...
| India Post | `postalpincode.in` | 0.1.1 | 2 | all verified | none |
| MDN Web Docs | `developer.mozilla.org` | 0.2.0 | 3 | all verified | none |
| Nominatim (OpenStreetMap) | `openstreetmap.org` | 0.1.0 | 2 | all verified | none |
| npm | `npmjs.com` | 0.1.1 | 17 | all verified | none |
| NSE | `nseindia.com` | 0.3.1 | 4 | all verified | none |
| Open Brewery DB | `openbrewerydb.org` | 0.1.1 | 5 | all verified | none |
| Open Library | `openlibrary.org` | 0.2.1 | 4 | all verified | none |
| Open-Meteo | `open-meteo.com` | 0.2.0 | 4 | all verified | none |
| PyPI | `pypi.org` | 0.1.0 | 18 | all verified | none |
| RBI | `rbi.org.in` | 0.3.0 | 3 | all verified | none |
| Read the Docs | `readthedocs.org` | 0.2.0 | 4 | all verified | none |
| SEBI | `sebi.gov.in` | 0.1.0 | 1 | all verified | none |
| SEC EDGAR | `sec.gov` | 0.2.1 | 4 | all verified | none |
...
| `add_address` | Add and select a delivery address for the current Amazon checkout. This is an ordinary confirmed account muta (write, `--confirm` required; login required) | unverified |
| `add_to_cart` | Fetch fresh product tokens and add an item. Defaults to the user's account cart and requests sign-in lazily. (write, `--confirm` required) | shape-verified |
...
| `batch_products` | Fetch compact current title, price, sale, image, and addability data for up to 25 ASINs in two direct request | shape-verified |
| `best_sellers` | Read Amazon's public best-seller list, optionally for a department slug. | shape-verified |
| `cart` | Read the current profile cart, including whether it belongs to an account or is an isolated anonymous cart th | shape-verified |
...
| `checkout_url` | Return Amazon's checkout URL. Use checkout for structured inspection or explicitly confirmed order placement. | unverified |
...
| `deals` | Search Amazon's Today's Deals index. The query is applied by Amazon upstream rather than by a local title sub | shape-verified |
| `list` | Read one Amazon List and the product cards currently rendered on it. (login required) | shape-verified |
| `lists` | List the signed-in account's Amazon Lists with IDs, privacy, default-list state, suspected QA artifacts, and (login required) | shape-verified |
| `orders` | Read recent order cards using a signed-in imported session. Never modifies orders. Amazon may require a separ (login required) | unverified |
| `payment_methods` | List payment methods exposed by the current checkout without returning Amazon's opaque instrument tokens. Dis (login required) | unverified |
| `product` | Fetch current product, price, availability, rating, sale, seller, and cart metadata. | shape-verified |
| `recommendations` | List related products directly linked from a public product page, excluding detected credit-card and account | unverified |
...
| `remove_from_list` | Remove an item from a specific Amazon List (wishlist) by list_id and item_id, then confirm by reading the lis (write, `--confirm` required; login required) | unverified |
| `reviews` | Read the public featured customer reviews visible on a product page. Amazon currently redirects its full sort | unverified |
| `sale_check` | Check whether a product currently displays a discount or lower price. | unverified |
| `search` | Search Amazon products and return compact structured results. Sponsored products are excluded by default and | unverified |
...
| Operation | Description | Status |
| --- | --- | --- |
| `download_pdf` | Download the rendered PDF for a known public arXiv paper and return a local file receipt with an integrity di | verified |
| `download_source` | Download the public source bundle for a known arXiv paper and return a local file receipt with an integrity d | verified |
...
| `list_author_papers` | Find public arXiv papers credited to a named author and return the newest matching records with normalized me | verified |
| `list_bulk_data_resources` | List official arXiv bulk-access resources for metadata, PDFs, source archives, terms, and incremental harvest | verified |
| `list_categories` | List arXiv's complete public archive and subject-category catalog with canonical browsing links. | verified |
| `list_category_papers` | List the newest public papers in an exact arXiv subject category with normalized metadata and stable offset p | verified |
...
| `search_papers` | Search the complete public arXiv corpus with its expressive query language and return normalized paper metada | verified |
...
| `corporate_actions` | List a BSE-listed company's corporate actions (dividends, splits, bonuses, schemes) with ex-dates and record | verified |
...
<details>
<summary><b>CoinGecko</b> (coingecko.com) — 3 operations</summary>

| Operation | Description | Status |
| --- | --- | --- |
| `market_chart` | Historical price, market cap, and volume series for one coin over a chosen number of days. | verified |
...
| `search_works` | Search Crossref's public scholarly metadata for works and return bounded matches with DOIs, titles, and citat | verified |
...
| Operation | Description | Status |
| --- | --- | --- |
| `add_reaction` | Add an emoji reaction to an issue or pull request via the authenticated REST API. Requires a token and confir (write, `--confirm` required; login required) | unverified |
| `api_get` | Bounded read-only escape hatch for public GitHub REST endpoints not yet represented by a typed command. Paths | unverified |
| `api_request` | Call any fixed-host GitHub REST or GraphQL endpoint. GET and HEAD are read-only; POST, PUT, PATCH, and DELETE (write, `--confirm` required) | unverified |
| `auth_status` | Report website-session and API-token authentication separately, including API quota tier and the age/freshnes | unverified |
| `branches` | List public repository branches and protection state. | shape-verified |
| `comment` | Add a comment to an issue or pull request via the authenticated REST API. Requires a token and confirm. (write, `--confirm` required; login required) | unverified |
...
| `create_gist` | Create a gist for the authenticated user via the REST API. Requires a token and confirm. (write, `--confirm` required; login required) | unverified |
| `create_issue` | Create an issue using the retained website login or a configured API token; either credential works. Requires (write, `--confirm` required; login required) | unverified |
| `create_pull_request` | Open a pull request through the authenticated GitHub REST API using a configured fine-grained token. Fully br (write, `--confirm` required; login required) | unverified |
| `create_release` | Create a release (optionally draft or prerelease) for a repository via the authenticated REST API. Requires a (write, `--confirm` required; login required) | unverified |
| `disconnect` | Delete the GitHub token stored in this AgentWeb profile. Environment-provided tokens are unchanged. (write, `--confirm` required) | unverified |
| `dispatch_workflow` | Trigger a workflow_dispatch run for a GitHub Actions workflow via the authenticated REST API. Requires a toke (write, `--confirm` required; login required) | unverified |
| `fork_repository` | Fork a repository into the authenticated user's account via the REST API. Requires a token and confirm. (write, `--confirm` required; login required) | unverified |
...
| `merge_pull_request` | Merge an open pull request via the authenticated REST API. Requires a token and confirm. Merging is irreversi (write, `--confirm` required; login required) | unverified |
...
| `review_pull_request` | Submit a pull request review (APPROVE, REQUEST_CHANGES, or COMMENT) via the authenticated REST API. Requires (write, `--confirm` required; login required) | unverified |
| `search_repositories` | Search public GitHub repositories with compact metadata and rate-limit status. Without github.configure_token | unverified |
| `set_star` | Star or unstar a repository for the authenticated user via the REST API. Requires a token and confirm. (write, `--confirm` required; login required) | unverified |
| `tags` | List public repository tags and archive links. | shape-verified |
| `update_issue` | Edit an issue or pull request (title, body, state, or state_reason) via the authenticated REST API. Close or (write, `--confirm` required; login required) | unverified |
| `user` | Fetch one public GitHub user profile. | unverified |
| `website_graphql` | Replay a persisted operation used by GitHub's current website. Asset discovery has strict request and time bu (write, `--confirm` required; login required) | shape-verified |
...
| `list_due_dates` | Return the GST portal's current filing, statement, refund, and quarterly compliance deadlines as structured d | verified |
...
| `list_gst_statistics` | List every official GST registration, return, collection, settlement, e-way-bill, and yearwise statistical do | verified |
| `list_help_resources` | List every public GSTN help, support, grievance, system-requirement, and taxpayer-facility resource with its | verified |
...
| `list_states` | List GST state and union territory codes, names, and territory classification for other state-scoped operatio | verified |
| `search_gst_practitioners` | Search the public GST practitioner register by state, optional name, district, or PIN code, without exposing | verified |
...
| `api_get` | Bounded read-only escape hatch for official Hacker News Firebase or Algolia endpoints. Hosts are fixed and on | unverified |
| `comment` | Post a comment or reply by extracting the current hidden HN form fields and submitting them directly. Returns (write, `--confirm` required; login required) | unverified |
...
| `edit` | Edit an eligible owned item through HN's current edit form. Requires confirm. (write, `--confirm` required; login required) | unverified |
| `favorite` | Favorite or unfavorite an item by extracting and replaying the current authenticated action URL. Requires con (write, `--confirm` required; login required) | shape-verified |
| `flag` | Flag or unflag an eligible item through its current authenticated action link. Requires confirm. (write, `--confirm` required; login required) | unverified |
| `item` | Fetch one story or comment with live score and comment totals from official Firebase plus a bounded comment t | unverified |
| `max_item` | Return the newest allocated Hacker News item ID. | unverified |
| `search` | Search Hacker News stories and comments by relevance or date using compact text previews; call hn.item for a | unverified |
...
| `submit` | Submit a link or text post by fetching HN's current hidden submission form token and posting directly. Exactl (write, `--confirm` required; login required) | unverified |
...
| `vote` | Vote or unvote by fetching the current item page, extracting HN's per-action auth URL, and replaying it direc (write, `--confirm` required; login required) | shape-verified |
...
<details>
<summary><b>MDN Web Docs</b> (developer.mozilla.org) — 3 operations</summary>

| Operation | Description | Status |
| --- | --- | --- |
| `compat` | Browser compatibility data for one web platform feature from MDN's BCD dataset. | verified |
| `page` | Read one MDN documentation page as plain-text sections by its slug. | verified |
| `search` | Full-text search across MDN Web Docs for web platform documentation. | verified |
...
| `get_version` | Resolve an exact version or distribution tag and return its normalized manifest, dependencies, and integrity | verified |
...
| `list_dependents` | List packages that npm currently shows as depending on a public package, using npm's browser-compatible publi | verified |
...
| `list_versions` | List a package's releases in publication order with dates, deprecations, manifests, and distribution integrit | verified |
| `read_package_file` | Read one bounded regular file directly from a public npm tarball as UTF-8 text or base64. | verified |
| `read_site_resource` | Read a bounded public npmjs.com policy, product, help, or docs.npmjs.com page without opening a browser. | verified |
| `search_packages` | Search the public npm registry with npm's text, keyword, maintainer, and scope query syntax. | verified |

</details>

<details>
<summary><b>NSE</b> (nseindia.com) — 4 operations</summary>

| Operation | Description | Status |
| --- | --- | --- |
| `announcements` | List the latest corporate announcements filed with NSE by listed companies, with subjects, timestamps, and at | verified |
...
| `indices` | Live snapshot of all NSE indices with last value, change, and 52-week range. | verified |
...
<details>
<summary><b>Open Library</b> (openlibrary.org) — 4 operations</summary>

| Operation | Description | Status |
| --- | --- | --- |
| `author_get` | Read one Open Library author record by author id, including name and birth date. | verified |
| `isbn` | Look up one book edition by its ISBN-10 or ISBN-13. | verified |
...
| Operation | Description | Status |
| --- | --- | --- |
| `air_quality` | Current air quality (PM2.5, PM10, US AQI) for a coordinate. | verified |
...
| `get_core_metadata` | Read the PEP 658 core metadata (the packaged METADATA file) for one distribution without downloading the whol | verified |
...
| `get_project` | Fetch a public PyPI project's normalized metadata: latest version, summary, license, classifiers, project URL | verified |
...
| `read_site_resource` | Read a bounded public pypi.org informational page (help, policy, or project page) as visible text and links. | verified |
...
| `press_releases_by_year` | List RBI press releases for a given calendar year from the archive. | verified |
| `press_releases_feed` | List the Reserve Bank of India's newest press releases from its official RSS feed. | verified |

</details>

<details>
<summary><b>Read the Docs</b> (readthedocs.org) — 4 operations</summary>

| Operation | Description | Status |
| --- | --- | --- |
| `builds` | Recent documentation builds for one Read the Docs project, newest first. | verified |
| `project` | Metadata for one Read the Docs project: language, homepage, repository, and default version. | verified |
| `search` | Server-side full-text search inside a specific project's documentation on Read the Docs. | verified |
| `versions` | Documentation versions available for one Read the Docs project. | verified |
...
<details>
<summary><b>SEC EDGAR</b> (sec.gov) — 4 operations</summary>
...
| `filings` | List a company's most recent SEC filings from its EDGAR submissions history. | verified |
...
| `add_to_queue` | Add a track or episode URI to the playback queue. Requires Premium. (write, `--confirm` required; login required) | shape-verified |
| `api_request` | Bounded authenticated escape hatch for any Spotify Web API path. The host is fixed to api.spotify.com; writes (write, `--confirm` required; login required) | unverified |
| `auth_status` | Validate the retained normal Web Player login or optional official OAuth, and report local desktop playback a | unverified |
| `configure` | Optional: save a Spotify developer Client ID for the official PKCE transport. Normal Web Player login through (write, `--confirm` required) | unverified |
| `create_playlist` | Create and live-verify a public, private, collaborative, or described playlist through the retained Web Playe (write, `--confirm` required; login required) | shape-verified |
...
| `devices` | List available Spotify Connect devices. Device IDs should be refreshed instead of cached indefinitely. (login required) | shape-verified |
| `disconnect` | Delete the local Spotify OAuth tokens for this AgentWeb profile. Does not revoke the app grant at Spotify. (write, `--confirm` required) | unverified |
...
| `next` | Skip to the next item. Requires Premium. (write, `--confirm` required; login required) | shape-verified |
| `pause` | Pause playback on the active or selected device. Requires Premium. (write, `--confirm` required; login required) | shape-verified |
| `play` | Play a query, track URI, or full album/artist/playlist context. Uses local Spotify Desktop with no setup or t (write, `--confirm` required; login required) | shape-verified |
...
| `previous` | Skip to the previous item. Requires Premium. (write, `--confirm` required; login required) | unverified |
| `queue` | Get the current playback queue. (login required) | unverified |
| `recently_played` | Return Spotify Web Player Recents activity after a normal login. Exact played_at cursor history is available (login required) | shape-verified |
...
| `repeat` | Set repeat mode to track, context, or off. Requires Premium. (write, `--confirm` required; login required) | shape-verified |
...
| `save_library` | Save or follow Spotify URIs through the retained Web Player library mutation, then verify live membership. Of (write, `--confirm` required; login required) | unverified |
| `search` | Search tracks without setup through public indexing, or search all catalog types through one normal retained | shape-verified |
| `seek` | Seek the active item to a millisecond position. Requires Premium. (write, `--confirm` required; login required) | unverified |
...
| `shuffle` | Enable or disable shuffle. Requires Premium. (write, `--confirm` required; login required) | shape-verified |
| `top_items` | Return the current user's top tracks or artists. (login required) | unverified |
| `transfer_playback` | Transfer playback to one Spotify Connect device and optionally play. Requires Premium. (write, `--confirm` required; login required) | unverified |
| `update_playlist` | Change and live-verify a playlist's name, visibility, collaboration, or description through the retained Web (write, `--confirm` required; login required) | shape-verified |
| `volume` | Set playback volume from 0 through 100. Requires Premium and a device that supports volume. (write, `--confirm` required; login required) | unverified |
...
| `add_comment` | Add a comment to a question or answer with automatic fkey handling. Requires confirm. (write, `--confirm` required; login required) | unverified |
| `answer` | Post an answer with automatic fkey handling. Requires confirm. (write, `--confirm` required; login required) | unverified |
...
| `ask` | Ask a question through Stack Overflow's current authenticated form protocol. AgentWeb obtains fkey automatica (write, `--confirm` required; login required) | unverified |
...
| `question_timeline` | List public timeline events for a question and explicitly identify vote aggregates whose direction and count | unverified |
| `questions` | List Stack Overflow questions. The hot option reads and parses the website's current Trending page directly, | unverified |
...
| `submit_form` | Submit any current same-site HTML form, including edit, save, flag, and moderation forms, by selector. Hidden (write, `--confirm` required; login required) | unverified |
...
| `vote` | Upvote, downvote, accept, unaccept, or undo a vote with automatic fkey handling. Site privileges still apply; (write, `--confirm` required; login required) | unverified |
...
| `api_query` | Bounded read-only escape hatch for MediaWiki query, parse, opensearch, and help actions. Write actions are re | unverified |
| `api_request` | Call a fixed-host MediaWiki Action API operation. Read actions use GET; mutations use POST, require confirm, (write, `--confirm` required) | unverified |
...
| `edit` | Create or replace a Wikipedia page through the MediaWiki Action API. AgentWeb fetches the CSRF token automati (write, `--confirm` required; login required) | unverified |
...
| `upload` | Upload a local file through MediaWiki's multipart Action API. AgentWeb fetches the CSRF token automatically a (write, `--confirm` required; login required) | unverified |
